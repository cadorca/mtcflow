 ...
 * @generated
 */
public class T2MTransformationGraphicalNodeEditPolicy extends GraphicalNodeEditPolicy {
	...
	protected Command getConnectionCompleteCommand(CreateConnectionRequest request) {
		...
		if (MtcElementTypes.OutputModel_4028 == elementType) {
		...
		if (MtcElementTypes.T2MTransformationInputFiles_4035 == elementType) {
			if (request.getSourceEditPart() != getHost()) {
				return getConnectionWithReorientedViewCompleteCommand(request);
			}
		}
		if (MtcElementTypes.TransformationModelDependencies_4040 == elementType) {
			if (request.getSourceEditPart() != getHost()) {
				return getConnectionWithReorientedViewCompleteCommand(request);
			}
		}
		if (MtcElementTypes.TransformationFileDependencies_4041 == elementType) {
			if (request.getSourceEditPart() != getHost()) {
				return getConnectionWithReorientedViewCompleteCommand(request);
			}
		}
