...
public class MTCFlowAcceleoCompiler {
	...
	/**
	 * Indicates if we should use binary resources for the serialization of the
	 * EMTL files.
	 */
	...
	/**
	 * Sets the boolean indicating if Acceleo should trim the positions from the
	 * emtl.
	 * 
	 * @param trimPosition
	 *            The boolean
	...
	/**
	 * This method will compute add the emtls from the jar in the list of
	 * dependencies to be loaded for the compilation and map their URIs to the
	 * logical URIs used by Acceleo.
	 * 
		...
		for (File mtlFile : mtlFiles) {
			...
			if (mtlFileAbsolutePath != null) {
				...
				if (outputFolder != null) {
					...
					String temp = new Path(mtlFileAbsolutePath).removeFileExtension().addFileExtension(IAcceleoConstants.EMTL_FILE_EXTENSION).toString();
					...
					emtlAbsoluteURI = URI.createFileURI(new Path(mtlFileAbsolutePath).removeFileExtension().addFileExtension(IAcceleoConstants.EMTL_FILE_EXTENSION).toString());
				}
	...
	/**
	 * Computes recursively the members of the given container that match the
		...
		if (container != null && container.isDirectory()) {
			...
			if (children != null) {
				for (File child : children) {
					if (child.isFile() && child.getName() != null && (extension == null || child.getName().endsWith('.' + extension))) {
	...
	/**
	 * Advanced resolution mechanism. There is sometimes a difference between
	 * how you want to load/save an EMTL resource and how you want to make this
	 * resource reusable.
	 * 
	 * @param dependenciesURIs
	 *            URIs of the dependencies that need to be loaded before link
	 *            resolution
	 * @param mapURIs
	 *            Advanced mapping mechanism for the URIs that need to be loaded
	 *            before link resolution, the map key is the loading URI, the
	 *            map value is the proxy URI (the real way to reuse this
	 *            dependency)
	 */
	private void computeDependencies(List<URI> dependenciesURIs, Map<URI, URI> mapURIs) {
		...
		for (Iterator<File> dependenciesIt = dependencies.iterator(); dependenciesIt.hasNext() && identifiersIt.hasNext();) {
		...
		for (File requiredFolder : dependencies) {
			if (requiredFolder != null && requiredFolder.exists() && requiredFolder.isDirectory()) {
				...
				for (File ecoreFile : ecoreFiles) {
					...
					AcceleoPackageRegistry.INSTANCE.registerEcorePackages(ecoreURI.toString(), AcceleoDynamicMetamodelResourceSetImpl.DYNAMIC_METAMODEL_RESOURCE_SET);
				}
			}
		}
	}
	...
	/**
	 * We may be calling for the compilation in standalone mode. In such a case
	 * we need a little more initialization.
	 */
	private void standaloneInit() {
		...
		if (registry.getContentTypeToFactoryMap().get(IAcceleoConstants.XMI_CONTENT_TYPE) == null) {
			registry.getContentTypeToFactoryMap().put(IAcceleoConstants.XMI_CONTENT_TYPE, new EMtlBinaryResourceFactoryImpl());
		}
	...
	protected void registerResourceFactories() {
		...
		Resource.Factory.Registry.INSTANCE.getContentTypeToFactoryMap().put(IAcceleoConstants.BINARY_CONTENT_TYPE, new EMtlBinaryResourceFactoryImpl());
		Resource.Factory.Registry.INSTANCE.getContentTypeToFactoryMap().put(IAcceleoConstants.XMI_CONTENT_TYPE, new EMtlResourceFactoryImpl());
	}
	...
	protected void registerLibraries() {
		...
		if (acceleoModel != null) {
			...
			URIConverter.URI_MAP.put(URI.createURI("http://www.eclipse.org/acceleo/mtl/3.0/mtlstdlib.ecore"), URI.createURI(stdlib.toString())); //$NON-NLS-1$
			URIConverter.URI_MAP.put(URI.createURI("http://www.eclipse.org/acceleo/mtl/3.0/mtlnonstdlib.ecore"), URI.createURI(resource.toString())); //$NON-NLS-1$
		} else {
